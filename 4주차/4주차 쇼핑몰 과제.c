...
void displayStatus(int *stock, int *sales, char (*productNames)[100], int count);
void inputIndividualStock(int *stock, int count);
void inputIndividualSales(int *stock, int *sales, int count);
void productName(char (*productNames)[100], int count);
    ...
    while (1) {
        ...
        switch (choice) {
            ...
            case 3:
                ...
                break;
            case 4:
                productName(productNames, MAX_PRODUCTS);
                break;
            case 5:
                printf("프로그램을 종료합니다.\n");
                return 0;
...
void displayStatus(int *stock, int *sales, char (*productNames)[100], int count) {
    ...
    printf("가장 많이 판매된 상품 : ID %d, 상품명 : %s, 판매량 %d\n", max_id + 1, *(productNames + max_id), max_sales);
    printf("가장 적게 판매된 상품 : ID %d, 상품명 : %s, 판매량 %d\n", min_id + 1, *(productNames + min_id), min_sales);
